...
local function unpack_int16(b)
    if type(b) ~= 'table' or #b < 2 then return ERROR_VALUE end
...
function update()
    for idx = 0, num_pins - 1 do
        ...
        -- now read the register data and collect its value as a signed 16-bit integer
        local data = read_register_data()
        gcs:send_named_float('A' .. idx, unpack_int(data))
        gcs:send_text(MAV_SEVERITY_INFO, "A" .. idx .. ": " .. value)
    end
